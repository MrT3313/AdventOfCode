    ...
    for ri, row in enumerate(matrix):
        for ci, col in enumerate(row):
            ...
            if (
                ci >= 1 and 
                matrix[ri][ci - 1] != "#" and
                matrix[ri][ci - 1] not in ["^", ">", "<", "v"]
            ):
                neighbors.append((ri, ci - 1))
            if (
                ci < len(matrix[0]) - 1 and 
                matrix[ri][ci + 1] != "#" and
                matrix[ri][ci + 1] not in ["^", ">", "<", "v"]
            ):  
                neighbors.append((ri, ci + 1))
            ...
            for neighbor_row, neighbor_col in neighbors:
                ...
                if (current_position[0], current_position[1]) in [
                    (6,3),
                    (7,6),
                    (7,7),
                    (8,1),
            ...
            valid_neighbors = [result for result in neighbor_results if result]
            if valid_neighbors:
                valid_wall_poitions.extend(valid_neighbors)
            
    print(f'VALID WALL POSITIONS : {valid_wall_poitions}')
    return len(valid_wall_poitions)
